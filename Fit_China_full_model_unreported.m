...
alpha_c=1/14;
fc=1;
function [dxdt]=Eode(t,X,beta,thetap,sigma,phi,nu,fc,thetac,p,alpha,rho,T)
        %beta=p(1);
        %T=p(2);
...
%    
% td0=2:45;
     [t,Sol] = ode45(@(t,X)Eode(t,X,beta,thetap,sigma,phi,nu,fc,thetac,p,alpha,rho,T),td,X0);
...
ps0=1/sigma0*(ddr-(1-p0)/p0*phi1)
 paramguess=[I0g,Mc0g,betag,phi1,theta,sigma0,nu0,thetac0,p0,alpha0,I0cg,I0ccg,rho0,Tg];
 lb=zeros(1,length(paramguess));
 %lb(3)=betag*.25;
 %lb(end-4)=0.95;
 %lb(4)=T_eg;
 ub=[10*I0g,10*Mc0g,4/N/Tg,1,.5,4000,1,.1,.1,.1,10*I0cg,10*I0ccg,1,7];
% 
% td=0:7:Ng*7;
% l1=length(td)
% NG-2
...
save('parametersNp2FLwinc2ccR2et.mat','paramfit');
T=paramfit(14)
...
% 
 ...
 [tn,Soln] = ode45(@(t,X)Eode(t,X,beta,thetafit,sigmaf,phi,nu,fc,thetac,p,alpha,rho,T),td,X0);
