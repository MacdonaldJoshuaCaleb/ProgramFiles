...
function []= FitSimpleModelChinaDaily_weibullquar
close all
clear 
warning off;
...
function[z] = Solve_sys(param,td)
   ...
   CumSq(1) = M(tq(1))*sqwt;
    ...
    %CumSqi=CumSq;
       mass(1) = (wblcdf(1.5,aw,bw));
        for j = 2:length(tq)
            mass(j) =(wblcdf(j+.5,aw,bw)-wblcdf(j-.5,aw,bw));
...
%length([C,M(tq)])
    ...
    paramfitChinaWeibullQ = paramfitChina;
    save('paramfitChinaWeibullQ.mat','paramfitChinaWeibullQ')
    figure
    plot(tq,wblcdf(tq,aw,bw),tq,expcdf(tq,(1/14)),'linewidth',2)
    legend('weibull','exponential')
    title('CDF')
    
    figure
    plot(tq,wblpdf(tq,aw,bw),tq,exppdf(tq,(1/14)),'linewidth',2)
    legend('weibull','exponential')
    title('PDF')
end