function []= FitSimpleModelChinaDaily
close all
clear 
warning off;
...
paramguess = [I0g,(1.66014398889413e-08)*(1e8),1407.12998368299,0.391575667314593,2];
lb = [I0g*.1,2/T , 0 , .01, 0];
ub = [I0g*5,6/T , inf , 1, inf ];

M = M';
C = C';


%ag=14*2;  %ag=14 gives exponential distribution with rate 1/14
%bg=14/ag;
%ag*bg
mass=zeros(size(tq));
defs=zeros(size(tq));
...
function[z] = Solve_sys(param,td)
    ...
    sigma = param(3);
    phi = param(4);
    ag=14*param(5);  %ag=14 gives exponential distribution with rate 1/14
    bg=14/ag;
...
defs=zeros(size(tq));
   ...
   CumSq(1) = M(tq(1))*sqwt;
       ...
       mass(1) = (gamcdf(1.5,ag,bg));
        for j = 2:length(tq)
            mass(j) =(gamcdf(j+.5,ag,bg)-gamcdf(j-.5,ag,bg));
            defs(j) = mass(j)*CumSq(j);
           % z2(j) = sum(defs(1:j-1));
        end
    CumSqi=CumSq-defs;
   Cds = Cdd(ids);
...
%length([C,M(tq)])
    ...
    save('paramfitChina_gq.mat','paramfitChina')
    I0 = paramfitChina(1)
      R0b = paramfitChina(2)
    sigma = paramfitChina(3)
    phi = paramfitChina(4)
    ag = paramfitChina(5)*14
    bg = 14/ag
    bg*ag
    paramfitChina(6) = resnorm;
    paramfitChina(5)
    paramfitChinaGammaQ = paramfitChina;
    save('paramfitChinaGammaQ.mat','paramfitChinaGammaQ')
    save('tq.mat','tq')
    figure
    plot(tq,gampdf(tq,ag,bg))
    title('gamma pdf for fit parameter')
    figure
    plot(tq,gamcdf(tq,ag,bg))
    title('gamma cdf for fit parameter')
end