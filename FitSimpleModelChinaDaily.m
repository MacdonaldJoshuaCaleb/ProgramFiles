...
%   nLevel = .5;
%   for j = 1:length(Cfit)           
%   CSyn(j) = Cfit(j) + normrnd(0,Cfit(j)*nLevel.^2);
%             while CSyn(j) < 0
%                 CSyn(j) = Cfit(j) + normrnd(0,Cfit(j)*nLevel.^2);
%             end
%   end
%    paramguess = paramfitChina(1:4);
%    paramguess(2) = paramguess(2)/T;
%    lb = [paramguess(1)*.05, 2/T , 0, paramguess(4)*.05];
%    ub = [paramguess(1)*10,6/T, inf   ,paramguess(4)*10];
%     [paramfit,resnorm] = lsqcurvefit(@Solve_sys,paramguess,td,[CSyn,M(tq)*sqwt],lb,ub);
%     paramfit(5) = paramfit(2)*T*(1-paramfit(4));
%     paramfit(2) = paramfit(2)*T;
%     paramfitsChinaDaily(k,:) = paramfit;
%     if mod(k,100) == 0
%         fprintf('-----------------------')
%         fprintf('iteration %i of %i',k,10000)
%         fprintf('-----------------------')
%     end
%   end
%  save('paramfitsChinaDaily.mat','paramfitsChinaDaily')
end
