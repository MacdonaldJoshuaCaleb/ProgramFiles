...
Re1=(betag*(1-phi1)*Tg)*N
 ...
 paramguess=[I0g,Mc0g,psi0,betag/10,phi1,p0,Tg];
 lb=zeros(1,length(paramguess));
 lb(4)=betag*.05;
lb(5)=0.05;
 %lb(9)=0.95;
 lb(end)=T_e;
 ub=[10*I0g,10*Mc0g,2000,10/N/Tg,1,.2,7];
% 
% td=0:7:Ng*7;
% l1=length(td)
% NG-2
...
% 
save('parametersNp2Llsc2n2t1cu5cb2.mat','paramfit');
 psi=paramfit(3)
       beta=paramfit(4)
      %tau=paramfit(8)
        p=paramfit(6)
...
for i=1:Nr
   Re(i)=1/(H(i)+theta*Td(i))*((sicd(siran)-sicd(siran-1))*C(i+siran)-(1-theta)*(sicdCT(siran)-sicdCT(siran-1))*Td(i+siran)); 
    Reno(i)=1/(H(i)+theta*Td(i))*((sicd(siran)-sicd(siran-1))*H(i+siran)-(sicdCT(siran)-sicdCT(siran-1))*Td(i+siran));%traced infected from quarantined and traced to infected cdf
   % Renop(i)=1/(H(i))*(((sicd(siran)-sicd(siran-1))*C(i+siran)-(1-rho)*(sicdCT(siran)-sicdCT(siran-1))*Td(i+siran)));
     Renop(i)=1/(H(i))*(((sicd(siran)-sicd(siran-1))*C(i+siran)));
end
...
% plot(1:Nr,Re,'b',1:Nr,Reno,'r',1:Nc,R0,'k')
 ...
 figure(12)
 plot(1:Nr,Re,'b-',1:Nr,Renop,'b--',1:Nr,Ret(1:Nr),'g-',1:Nr,Retno(1:Nr),'g-')
