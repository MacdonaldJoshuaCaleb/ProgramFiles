...
function [dxdt]=Eode(t,X,beta,thetap,sigma,phi,nu,fc,thetac,p,alpha,rho)
   ...
   lambda=(beta*X(5)+betaM*X(6)+betac*X(11));
     ...
     dxdt(2)=(sigma)*X(1)*(lambda)-nu*(1+(1-p)/p*phi*rho)*X(2)*lambda-alpha*X(2)+fc*alpha_c*X(9);
     dxdt(3)= X(1)*(beta*(1-phi*rho)*X(5)+betaM*(1-phi*rho)*X(6)+betac*(1-phi)*X(11))- 1/tau*X(3);
     dxdt(4)= nu*(beta*(1-phi*rho)*X(5)+betaM*(1-phi*rho)*X(6)+betac*(1-phi)*X(11))*X(2)- 1/tau*X(4);
     dxdt(5)= 1/tau*X(3)-1/T*X(5);
     dxdt(6)=1/tau*X(4)-1/T_e*X(6);
     dxdt(7)=rho/T*X(5);
     dxdt(8)=rho/T_e*X(6);
     dxdt(9)=((1-p)/p*phi*rho)*X(1)*(lambda)+((1-p)/p*phi*rho)*X(2)*nu*(lambda)-alpha_c*X(9)-nu_c*X(9)*X(5)*lambda;
...
%      X0=[S0 Sc0 E0 Ec0 I0 0 C(1) Td(1) (1-p)*Mc0 p*Mc0 0 0 0];
     X0=[S0 Sc0 E0 Ec0 I0 0 C(1) 0 (1-p)*Mc0 p*Mc0 I0c Td(1) I0cc];
%    
% td0=2:45;
...
ps0=1/sigma0*(ddr-(1-p0)/p0*phi1)
 ...
 %lb(end-4)=0.95;
 %lb(4)=.2;
 ub=[10*I0g,10*Mc0g,1,.15,4000,.26,.16,.15,.1,10*I0cg,10*I0ccg];
% 
% td=0:7:Ng*7;
% l1=length(td)
% NG-2
...
save('parametersNp2FLwinc2ccR2eC2.mat','paramfit');
% betaf=paramfit(3)
betaf=beta
 phif=paramfit(3)
  %fc=paramfit(7)
