function []= FitSimpleModelAggFixp
close all
warning off;
...
lb = [I01g*.05, 2/T , 0 , .01 , I02g*.05, 2/T , 0 , .01 ];
ub = [I01g*10,6/T, inf , 1  ,I02g*10, 6/T , inf , 1 ];
...
function[z] = Solve_sys(param,td)
   ...
   CumSq=(Sol1(tq,4)+Sol1(tq,5)+Sol1(tq,9))*sqwt + (Sol2(tq,4)+Sol2(tq,5)+Sol2(tq,9))*sqwt;
   CumSq(1) = M(tq(1))*sqwt;
   Cds1=Cdd1(1:end-1);
   Cds2 = Cdd2(tds);
   z=[Cds1',Cds2',CumSq'];
    ...
    figure
    plot(tds,Cfit2,'b-',td(1:end-1),Cd2,'r*','linewidth',2)
    title('Hubei')
    ylabel('New Daily Cases')
